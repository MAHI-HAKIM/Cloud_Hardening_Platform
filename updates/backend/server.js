...
app.get("/api/ssh/stream-connect", (req, res) => {
  ...
  try {
    const sshCheckCmd =
      process.platform === "win32" ? "where ssh" : "which ssh";
  ...
  try {
    const status = execSync("systemctl is-active ssh", {
      encoding: "utf-8",
    }).trim();
    send(
      status === "active"
        ? "✅ SSH service is running."
        : `⚠️ SSH service status: ${status}`
    );
  } catch {
  ...
  if (authMethod === "key") {
    ...
    command = `sshpass -p "${passwordOrKey}" ssh ${sshArgs.join(
      " "
...
app.get("/api/getplaybooks/available", (req, res) => {
  ...
  fs.readdir(baseDir, (err, vendors) => {
    if (err)
      return res.status(500).json({ error: "Error reading playbook folders." });
    const playbooks = [];
...
/**
 * 3. GET AVAILABLE AUDIT PLAYBOOKS
 */
app.get("/api/getplaybooks/audit", (req, res) => {
  const baseDir = path.join(__dirname, "ansible", "audit");
  const playbooks = [];
  fs.readdirSync(baseDir).forEach((vendor) => {
    const vendorDir = path.join(baseDir, vendor);
    fs.readdirSync(vendorDir).forEach((file) => {
      if (file.endsWith(".yml") || file.endsWith(".yaml")) {
        playbooks.push({
...
/**
 * 3b. GET AVAILABLE HARDENING PLAYBOOKS
 */
app.get("/api/getplaybooks/hardening", (req, res) => {
  const baseDir = path.join(__dirname, "ansible", "hardening");
  const playbooks = [];
  fs.readdirSync(baseDir).forEach((vendor) => {
    const vendorDir = path.join(baseDir, vendor);
    fs.readdirSync(vendorDir).forEach((file) => {
...
app.post("/api/ssh/run-playbook", async (req, res) => {
  ...
  if (!password && !sshKey) {
    return res
      .status(400)
      .json({ success: false, error: "No password or SSH key provided." });
  }

  if (!playbookPath || !fs.existsSync(playbookPath)) {
    return res
      .status(400)
      .json({ success: false, error: "Playbook path is invalid or missing." });
  }
  ...
  try {
    ...
    if (!sshOutput.includes("Connected")) {
      ...
      return res.status(500).json({
        success: false,
        error: "SSH test failed. Cannot proceed with playbook.",
      });
    }
...
app.post("/api/ssh/generate-keypair", (req, res) => {
  ...
  exec(`ssh-keygen -t rsa -b 2048 -f "${privateKeyPath}" -N ""`, (error) => {
    if (error)
      return res
        .status(500)
        .json({ success: false, error: "SSH key generation failed" });
    try {
      ...
      res.json({
        ...
        privateKey,
        publicKey,
        info: "Add this public key to your firewall's ~/.ssh/authorized_keys",
      });
    } catch {
      res
        .status(500)
        .json({ success: false, error: "Could not read generated keys" });
    }
  });
});
  ...
  try {
    ...
    if (!playbookPath || !fs.existsSync(path.join(__dirname, playbookPath))) {
      return res.status(400).json({
        success: false,
        error: "Playbook for selected standard not found.",
      });
    }
    ...
    const command = `ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook "${path.join(
      __dirname,
      playbookPath
    )}" -i "${host}," -u "${username}" --private-key "${tempKeyPath}"`;
